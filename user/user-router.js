...
router.post("/register", (req, res) => {
  ...
  Users.addUser(user)
    .then((saved) => {
      ...
      res
        .status(201)
        .json({ message: "Register Successful", user: saved, token: token });
      console.log(token);
    })
    .catch((err) => {
  ...
  Users.getUserAccount(id)
    ...
    .catch((err) => {
      res
        ...
        .json({
          error: err,
          message: `Error User id ${id} strains were not found`,
        });
    });
});
  ...
  Users.getUsers()
    ...
    .catch((err) => {
      res
        ...
        .json({
          error: err,
          message: `Error user id ${id} account was not found`,
        });
    });
});
...
router.get("/:id", restricted, (req, res) => {
  ...
  Users.findUserById(id)
    ...
    .catch((err) => {
      res
        .status(500)
        .json({ error: err, message: `Error user id ${id} was not found` });
    });
});
...
router.put("/:id", restricted, (req, res) => {
  ...
  Users.updateUser(id, updatedUser)
    ...
    .catch((err) => {
      res
        .status(500)
        .json({ error: err, message: `Error Strain id ${id} was not updated` });
    });
});
...
function generateToken(user) {
  ...
  const secret =
    process.env.JWT_SECRET || "penguins live in the desert to eat cactus";
  const options = {
    expiresIn: "8h",
  };
